...
module.exports.signIn = async (req, res) => {
  ...
  res
    .status(200)
    .json({ status: 200, message: "Login Success", success: true });
};
module.exports.signUp = async (req, res) => {
  let { fullname, email, number, password } = req.body;
  let profile_path = req.file
    ? `/uploads/${req.file.filename}`
    : process.env.DEFAULT_PROFILE_PATH;
  ...
  try {
    ...
    res
      .status(200)
      .json({ status: 200, message: "Account Created ", success: true });
  } catch (e) {
